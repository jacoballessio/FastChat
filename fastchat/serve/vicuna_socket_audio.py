...
def save_audio_file(client_socket, file_path):
    with open(file_path, 'wb') as audio_file:
        isMore = True
        while isMore:
            audio_data = client_socket.recv(1024)
            if audio_data and b'END_OF_AUDIO' not in audio_data:
...
def main(args):
    ...
    print("Cacheing whisper jax")
    pipeline("fastchat/serve/test_audio.mp3")
    print("Whisper is up and running")
    # Model
    if args.device == "cuda":
    ...
    while True:
        ...
        client_socket.sendall((final_token+"<|EOM|>").encode())  # Send the final token to the client
